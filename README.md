...
- **PairedReLU**: $f(x) = \left[\text{ReLU}(a x - b), \text{ReLU}(c x - d)\right]$ (concatenated along channel dimension)
- **RMAF**: $f(x) = \frac{a b x}{(0.25(1 + \exp(-x)) + 0.75)^c}$
- **PTELU**: $f(x) = \begin{cases} x & \text{if } x \geq 0 \\\\ \vert a \vert \tanh(\vert b \vert x) & \text{otherwise} \end{cases}$
- **TaLU**: $f(x) = \begin{cases} x & \text{if } x \geq \vert b \vert \\\\ \tanh(x) & \text{if } x > \vert a \vert \\\\ \tanh(\vert a \vert) & \text{otherwise} \end{cases}$
